...
ARTIFACT_NAME="$(
  docker run --rm "$IMAGE_TAG" sh -lc 'ls -1 /postgresql95-hashlib*.tar.gz 2>/dev/null | head -n1 | xargs -n1 basename'
)"

if [[ -z "$ARTIFACT_NAME" ]]; then
  echo "Error: Could not find artifact '/postgresql95-hashlib*.tar.gz' inside the image." >&2
